...
class Job(Job) :
	def gen_script(self) :
		...
		call_line += ( '-ex' , '"set environment TMPDIR=$TMPDIR"' ) # gdb keep the environment except TMPDIR
		if self.stdin or self.stdout :
			# in case of redirections, define alias r to run with adequate redirections
			run = 'run'
