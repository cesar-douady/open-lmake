...
class DirtyRule(Rule) :
	target_flags = { '__NO_MATCH__' : ('{*:.*}','Incremental','-Warning') }

class _PyRuleBase(Rule) :
	'base rule that handle pyc creation when importing modules in Python'
	def __init_subclass__(cls) :
		super().__init_subclass__()
class Py2Rule(_PyRuleBase) : target_flags = { '__PYC__' : ( r'{*:(.+/)?}{*:\w+}.pyc'                         , 'Incremental','ManualOk' ) } # for Python2
class Py3Rule(_PyRuleBase) : target_flags = { '__PYC__' : ( r'{*:(.+/)?}__pycache__/{*:\w+}.{*:\w+-\d+}.pyc' , 'Incremental','ManualOk' ) } # for Python3
