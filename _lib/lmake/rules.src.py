...
class _RuleBase :
	...
	# for dict, a value of None   discards the entry
	# for set , a key   of -<key> discards <key>
	# for list, entries are concatenated
	combine = {
		'stems'
...
#	target                                    # syntactic sugar for targets = {'target':<value>} (except job's stdout is redirected to it)
...
#	autodep             = 'ld_audit'                  # autodep method : none, ld_audit, ld_preload, ld_preload_jemalloc, ptrace, seccomp
...
#	chroot_actions      = ()                          # actions to carry out when chroot to chroot_dir, a list/tuple of : 'user_name', 'resolv_conf'
...
#	compression         = None                        # compression to use when caching :
	...
	#                                                 # - if a int : the compression level : between 0 (no compression) and 9 (best and slowest), method defaults to zstd if supported, else zlib
...
#	dep                                               # syntactic sugar for deps = {'dep':<value>} (except that job's stdin is redirected to it)
...
#	lmake_root          = '/path/to/open-lmake'       # absolute path of the open-lmake installation dir to be used by job (default is current installation dir)
...
#	max_runs            =   0                         # maximum number a job can be run in a single lmake command, unlimited if None or 0
	...
	max_submits         =  10                         # maximum number a job can be submitted in a single lmake command, unlimited if None or 0, above 10, the flow can probably be
	#                                                 # easily optimized
...
#	retried_errors      = ('-timeout',)               # kind of errors that are retried when --retry-on-error=<cnt> option is passed to lmake
...
#	timeout             = None                        # timeout allocated to job execution (in s), must be None or a float
...
class DirRule(Rule) :
	...
	def __init_subclass__(cls,**kwds) :
		super().__init_subclass__(**kwds)
		cls.target  = fr'{{Dir:.+}}/{cls.marker}' # adapt target to new marker
	virtual = True
	marker  = '...'
	target  = fr'{{Dir:.+}}/{marker}'
	backend = 'local'                             # command is faster than any remote backend overhead
	cmd     = ''
...
class ConfigRule(PyRule) :
	'''
		...
		For example, if config.py contains :
			config = { 'a':{'b':1} , c:[2,3] }
		then with cfg=lmake.config_dict('config') :
			- cfg.a()   =={'b':1}
			- cfg['a']()=={'b':1}
			- cfg.a.b() ==1
			- cfg.c()   ==[2,3]
