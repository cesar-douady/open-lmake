...
#	target            # syntactic sugar for targets = {'<stdout>':<value>} (except that it is allowed)
	target_flags = {} # patterns used to add flags based on pattern matching refering to stems above through {} notation, e.g. {'CACHE':'{File}.cache','incremental'}
	dep_flags    = {} # .
...
class DirtyRule(Rule) :
	target_flags = { '__NO_MATCH__' : ('{*:.*}','Incremental','NoWarning') }
...
class _PyRuleBase(Rule) :
	...
	dep_flags = { '__PY__' : ( r'{*:(.+/)?}{*:\w+}.pyc' , 'StatReadData' ) }                                                     # this is actually a noop as this is the default
class Py2Rule(_PyRuleBase) : target_flags = { '__PYC__' : ( r'{*:(.+/)?}{*:\w+}.pyc'                         , 'Incremental' ) } # for Python2
class Py3Rule(_PyRuleBase) : target_flags = { '__PYC__' : ( r'{*:(.+/)?}__pycache__/{*:\w+}.{*:\w+-\d+}.pyc' , 'Incremental' ) } # for Python3
