...
if __name__!='__main__' :
	...
	for z_lvl in (0,5) :
		...
		ut.lmake( 'hello+auto1.hide.ok' , done=2 , hit_rerun=1 , hit_done=2 ,                 new=2 ) # check cache hit on common part (except auto1), and miss when hello is dep
		ut.lmake( 'mkdir.dut.ok'        , done=1 , hit_rerun=1 , hit_done=1 , quarantined=1 , new=1 ) # check all is ok with dirs and empty files (mkdir.dut still exists and is unlinked)
		...
		ut.lmake( 'mkdir.dut.ok'        , done=1 , hit_rerun=1 , hit_done=1 , quarantined=1 , new=1 )
