...
,	Static                             // is static           , for internal use only, only if also a Target (could be merged with IsDep )
,	Target                             // is a target         , for internal use only
,	IsDep                              // target is also a dep, for internal use only, only if not  a Target (could be merged with Static)
)
...
ENUM( ExtraTflag
,	Top
,	ReadIsDep // target is a dep if read
...
struct AccessDigest {                                                  // order is first read, first write, last write, unlink
	...
	// data
	Bool3       write        = No ;                                    // if Yes, files are written, if Maybe files are unlinked
	Accesses    accesses     = {} ;
...
public :
	...
	JobExecRpcReq( P p ,                   ::string&& t={} ) : proc{p} ,                                                            txt{::move(t)} { SWEAR( p<P::HasFiles && p!=P::Confirm ) ; }
	JobExecRpcReq( P p , bool sc         , ::string&& t={} ) : proc{p} , sync{sc&&p!=P::Confirm} , confirm{No|sc|(p!=P::Confirm)} , txt{::move(t)} { SWEAR( p<P::HasFiles                  ) ; }
	JobExecRpcReq( P p , bool s , bool c , ::string&& t={} ) : proc{p} , sync{s                } , confirm{No|c                 } , txt{::move(t)} { SWEAR(                  p==P::Confirm ) ; }
	//
private :
	JobExecRpcReq( P p , bool ad , ::string&& cwd_ , ::vmap_s<DD>&& fs , AccessDigest const& d , bool nf , bool s , ::string&& comment ) :
		proc     { p               }
	,	sync     { s               }
	,	auto_date{ ad              }
	,	no_follow{ nf              }
...
public : //!                                                                                                   auto_date    cwd                             no_follow sync
...
public :
	template<IsStream T> void serdes(T& s) {
		...
		switch (proc) {
			...
			case P::Panic    :
			case P::Guard    :                                             break ;
			case P::Confirm  :                       ::serdes(s,confirm) ; break ;
			case P::Access   : ::serdes(s,digest ) ; ::serdes(s,confirm) ; break ;
			case P::DepInfos : ::serdes(s,digest ) ;                       break ;
