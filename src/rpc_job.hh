...
struct JobExecRpcReq {
	...
	struct AccessDigest {                                                      // order is read, then write, then unlink
		...
		// accesses
		bool idle() const { return write==No && unlink==No ; }                 // for state management, handle Maybe as Yes
		// services
		...
		void confirm( bool ok                           ) {
			if (write ==Maybe) write  = No|ok ;
			if (unlink==Maybe) unlink = No|ok ;
		}
		// data
		...
		Bool3    write      = No ;     // if true <=> files are written, possibly unlinked later
		Bool3    unlink     = No ;     // if true <=> files are unlinked at the end, possibly written before
	} ;
	// statics
private :
...
public :
	...
	,	comment  {c         }
	{ SWEAR(p==P::Access) ; }
	JobExecRpcReq( P p , ::vector_s&& fs , bool ok_ ) :
		proc {p                    }
	,	ok   {ok_                  }
	,	files{_s_mk_mdd(::move(fs))}
	{ SWEAR(p==P::Confirm) ; }
	...
	template<IsStream T> void serdes(T& s) {
		...
		switch (proc) {
			...
			case P::Access   :
				...
				::serdes(s,comment) ;
			break ;
				::serdes(s,files) ;    // dates are not significative
				::serdes(s,ok   ) ;
			break ;
			default :
				::serdes(s,comment) ;
