...
inline bool is_blocked_sig(int sig) {
	sigset_t old_mask ;
	int      rc       = ::pthread_sigmask( 0/*how*/ , nullptr/*set*/ , &old_mask ) ;
	SWEAR( rc==0 , "cannot get sig ",sig ) ;
	return sigismember(&old_mask,sig) ;                                                        // sigismember can be a macro, hence no ::
}
inline void block_sigs  (::vector<int> const& sigs) { swear( ::pthread_sigmask( SIG_BLOCK   , &::ref(_mk_sigset(sigs)) , nullptr/*oldset*/ )==0 , "cannot block sigs "  ,sigs) ; }
