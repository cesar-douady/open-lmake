...
namespace Backends::Local {
	...
	public :
		...
		::pair_ss lacking_rsrc( RsrcsData const& rs             ) const override {
			for( size_t i : iota(rs.size()) )
				if (rs[i]>capacity_[i]) {
					return {
						cat("not enough resource ",rsrc_keys[i]," (required ",rs[i]," but only ",capacity_[i]," available)") // msg
					,	cat("lmake.config.backends.",MyTag,'.',rsrc_keys[i]," = ",rs[i]                                    ) // consider
					} ;
				}
			return {} ;
		}
		...
		::pair_s<HeartbeatState> heartbeat_queued_job( Job job , SpawnedEntry const& se ) const override {  // called after job_exec has had time to start
			...
			if (wstatus_ok(wstatus)                 ) return { read_stderr(job) , HeartbeatState::Lost  } ; // process died long before (already waited) or just died with no error
			/**/                                      return { read_stderr(job) , HeartbeatState::Err   } ; // process just died with an error
		}
	...
	inline RsrcsData::RsrcsData( ::vmap_ss const& m , ::umap_s<size_t> const& idxs , bool rnd_up ) {
		...
		for( auto const& [k,v] : m ) {
			auto it = idxs.find(k) ;
			if (it==idxs.end()) {
				::string msg = cat("no resource ",k," for backend ",MyTag,'\n') ;
				try                     { Rsrc n=from_string_with_unit<Rsrc,false/*RndUp*/>(v) ; msg << "  consider : lmake.config.backends.local."<<k<<" = "<<n ; }
				catch (::string const&) {                                                        msg << "  consider setting rule backend attribute"              ; }
				throw msg ;
			}
