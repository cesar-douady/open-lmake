...
namespace Engine {
	...
	NodeReqInfo::NodeReqInfo( Req req , Node n ) : ReqInfo{req} {
		if (!n) return ;
		NodeData& nd = *n ;
		nd.set_buildable() ;
	...
	Manual NodeData::manual_wash( ReqInfo& ri , bool query , bool dangling ) {
		...
		Req    req   = ri.req                          ;
		Manual res   = manual_refresh(~Accesses(),req) ;
		bool   stamp = true                            ;
	...
	bool/*modified*/ NodeData::refresh_src_anti( ::string const& name_ , Accesses a , bool report_no_file , bool keep_actual_job , ::vector<Req> const& reqs_ ) { // reqs_ are for reporting only
		...
		Trace trace("refresh_src_anti",idx(),STR(report_no_file),a,reqs_,sig_) ;
		...
		if (!fi.exists()) {
			...
			if (crc.match(Crc::None,a)) return false/*updated*/ ;
			//vvvvvvvvvvvvvvvvvvvvv
			set_crc_date(Crc::None) ;
			//^^^^^^^^^^^^^^^^^^^^^
			return true/*updated*/ ;
		} else {
			bool sig_ok = sig_==sig.sig || Crc(sig_.tag()).match(Crc(sig.sig.tag()),a) ; // sig's match or are enough to match content
			if ( crc.valid() && sig_ok ) return false/*updated*/ ;
			Crc  crc_    { name_ , /*out*/sig_ }    ;
			bool updated = +crc.diff_accesses(crc_) ;
			//vvvvvvvvvvvvvvvvvvvvvvvvv
			set_crc_date( crc_ , sig_ ) ;
			//^^^^^^^^^^^^^^^^^^^^^^^^^
			if ( updated && !sig_ok && !keep_actual_job ) {
				actual_job = {} ;                                                      // if actively updated, it is not generated by a job any longer (unless asked so)
				for( Req r : reqs() )
					if (fi.date>r->start_ddate) req_info(r).overwritten = true ;
			}
			if (!actual_job) {                                                         // if a source has an actual job, it is dynamically created and not reported as source nor can it be overwritten
	...
	struct JobTgtIter {
		...
		bool          operator+ () const {                                                      return idx<node.n_job_tgts && _cur_prio()>=_prev_prio ; }
	private :
	...
	Buildable NodeData::_gather_prio_job_tgts( ::string&/*lazy*/ name_ , Req req , RejectSet&/*lazy*/ known_rejected , DepDepth lvl ) {
		...
		//
		RuleIdx           n_rules    = 0                ;
		Buildable         res        = Buildable::No    ;                                                       // return val if we find no job candidate
	...
	Done :
		if (+new_jts) {
			...
			job_tgts.shorten_by(job_tgts.size()-n_job_tgts) ;
			job_tgts.append(new_jts)                        ;
			//^^^^^^^^^^^^^^^^^^^^^^
			...
			try {                                                                                    // ... in case of crash, rescue mode is used and ensures all matches are recomputed
				...
				if (+dir) {
					...
					switch ((db=dir->buildable)) {
						...
						case Buildable::Codec     : buildable = Buildable::No        ; goto Return ; // dir cannot be a symlink
				...
				//          vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv
				buildable = _gather_prio_job_tgts( name_ , req , known_rejected , lvl ) ;
				//          ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
				if (db==Buildable::Maybe) buildable = ::max(buildable,Buildable::Maybe) ;            // if dir is buildable, uphill applies (and generates an error unless it is a symlink)
				goto Return ;
	...
	bool/*solved*/ NodeData::_make_pre( ReqInfo& ri , bool query ) {
		...
		SWEAR( buildable!=Buildable::Unknown , self,buildable ) ;
		// step 1 : handle what can be done without dir
		if ( buildable<=Buildable::No                   ) { set_status(NodeStatus::None) ; goto NoSrc   ; }
		if ( buildable>=Buildable::Yes && is_src_anti() ) { set_status(NodeStatus::Src ) ; goto Src     ; }
		if ( !dir                                       )                                  goto NotDone ;
			...
			for (;;) {
				...
				if (!ri.single) {                            // fast path : cannot have several jobs not gather new jobs if we consider only a single (existing) job
					...
					for( JobTgtIter it{self,ri} ; +it ; it++ ) {                                                 // check if we obviously have several jobs, in which case make nothing
						JobTgt jt = *it ;
						if      ( jt.sure()                 )   buildable = ::max(buildable,Buildable::Yes) ;    // buildable is data independent & pessimistic (may be Maybe instead of Yes)
	...
	void Dep::full_refresh( bool report_no_file , Job j , ::vector<Req> const& reqs ) const { // reqs are for reporting only
		...
		if (nd.is_src_anti()) {
			...
			bool set_actual_job = dflags[Dflag::Codec] && (create_encode||!nd.actual_job) ;
			if (set_actual_job) { SWEAR( +j , self ) ; nd.actual_job = j ; }
			nd.refresh_src_anti( nd.name() , accesses() , report_no_file , set_actual_job , reqs ) ;
		} else {
