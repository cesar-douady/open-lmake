...
namespace Codec {
	...
	static ::map_s/*ctx*/<::map_ss/*val->code*/> _prepare_encode_tab( ::vector_s const& lines , Bool3&/*out*/ has_new_codes ) {
	...
	static ::map_s/*ctx*/<::map_ss/*code->val*/> _mk_decode_tab(::map_s/*ctx*/<::map_ss/*val->code*/> const& encode_tab) {
		Trace trace(CodecChnl,"_mk_decode_tab",encode_tab.size()) ;
		...
		trace("done",n_ctxs,n_entries,res.size()) ;
		return res ;
	}
...
namespace Engine {
	...
	RestartFullAnalysis :
		...
		// roughly equivalent to !jrt||jrt>=Err but give reason and take care of limits
		auto no_run_reason_tag = [&]( JobReasonTag jrt , Bool3 has_run ) {                           // Maybe means was submitted but did not run
			switch (jrt) {
				case JobReasonTag::None      :                             return NoRunReason::Dep ; // reason for not rerun is that deps are ok
		...
		auto inc_submits = [&]( JobReasonTag jrt , bool has_run ) {                                  // inc counter associated with no_run_reason_tag (returning None)
			NoRunReason nrr = no_run_reason_tag(jrt,Maybe|has_run) ;
		...
		RestartAnalysis :                                                                             // restart analysis here when it is discovered we need deps to run the job
			...
			RestartDep :
				...
				if (cdri->waiting()) {
					...
					dnd.add_watcher(*dri,job,ri,dep_pressure) ;
	...
	Wakeup :
		if ( auto it = req->missing_audits.find(job) ; it!=req->missing_audits.end() && !req.zombie() ) {
			...
			AuditPfx  pfx =
				status==Status::RunLoop    ? AuditPfx::None
			:	status==Status::SubmitLoop ? AuditPfx::None
			:	ja.report==JobReport::Hit  ? AuditPfx::Hit
			:	                             AuditPfx::Was
			;
			MsgStderr mse ;
			::string  crt ;
		...
		if ( ri.done() && wakeup_watchers ) {
			if (special_!=Special::Dep) {
				...
				to_pop.job = job ; // once reply is sent, we can dispose of ourselves (dont do ToPop assignment to avoid 2 destructions)
				to_pop.req = req ;
			}
		}
		report_reason = reason(ri.state) ;
	...
	::pair<bool/*maybe_new_deps*/,bool/*triggered*/> JobData::_submit_plain( ReqInfo& ri , CoarseDelay pressure ) {
		...
		for( Req rr : running_reqs() ) if (rr!=req) {
			...
			ri.set_step(cri.step(),job) ;                                              // Exec or Queued, same as other reqs
			ri.inc_wait() ;
