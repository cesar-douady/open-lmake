...
namespace Engine {
	...
	void Job::record(JobInfo1 const& ji) const {
		...
		AcFd( ancillary_file() , {O_WRONLY|(creat?O_TRUNC|O_CREAT:O_APPEND)} ).write( jaf ) ;
	}
	...
	void Job::record(JobInfo const& ji) const {
		...
		AcFd( ancillary_file() , {O_WRONLY|O_TRUNC|O_CREAT} ).write( jaf ) ;
	}
	...
	void JobExec::started( bool report , ::vmap<Node,FileActionTag> const& report_unlnks , MsgStderr const& msg_stderr ) {
		...
		SWEAR( self->is_plain() , self->special() ) ;
	...
	JobExec::EndDigest JobExec::end_analyze(JobDigest<Node>&/*inout*/ digest) {
		...
		if ( !lost && status>Status::Early ) {         // if early, we have not touched the targets, not even washed them, if lost, old targets are better than new ones
			...
			for( auto& [target,td] : digest.targets ) {
				...
				else if ( crc.exists() && crc!=target->crc ) target->polluted = Polluted::Job      ; // we pollute the official job
				//
				if (+target->polluted) trace("polluted",target->polluted) ;
				//
				if (!crc) {
					trace("keep_old") ;
			...
			for( auto [t,tf] : old_targets ) {
				if (!tf[Tflag::Incremental]) {
					...
					td.actual_tflags = {} ;                                 // .
					td.polluted      = {} ;                                 // .
					td.set_crc_date( Crc::None , {{}/*sig*/,start_date} ) ; // non-incremental old targets were unlinked at start-of-job time, during washing
				} else if (tf[Tflag::Target]) {
	...
	::string cache_repo_cmp( JobInfo const& info_cache , JobInfo const& info_repo ) {
		...
		for( auto const& [key,cache_repo] : diff_targets )
			...
			else if (!cache_repo.first .exists()         ) w = ::max( w , only_in_repo     .size() ) ;
			else if (!cache_repo.second.exists()         ) w = ::max( w , only_in_cache    .size() ) ;
			else                                           w = ::max( w , different_content.size() ) ;
		if (!w) return {} ;
		::string msg ;
		for( auto const& [key,cache_repo] : diff_targets )
			if      (cache_repo.first ==cache_repo.second) {}
			else if (!cache_repo.first .exists()         ) msg << widen(only_in_repo     ,w)<<" : "<<key<<'\n' ;
			else if (!cache_repo.second.exists()         ) msg << widen(only_in_cache    ,w)<<" : "<<key<<'\n' ;
