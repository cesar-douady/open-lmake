...
namespace Time {
	...
	void Delay::operator>>(::string& os) const {                                          // START_OF_NO_COV
		int64_t  s  =       sec      ()  ;
		uint32_t ns = ::abs(nsec_in_s()) ;
		/**/                os << "D:"                                                  ;
		if ( !s && _val<0 ) os << '-'                                                   ;
		/**/                os << cat(s,'.',widen(cat(ns),9,true/*right*/,'0'/*fill*/)) ;
	}                                                                                     // END_OF_NO_COV
	...
	void CoarseDelay::operator>>(::string& os) const { // START_OF_NO_COV
		os << Delay(self) ;
	}                                                  // END_OF_NO_COV
	...
	void Ddate::operator>>(::string& os) const { os << "DD:"<<str(9)<<':'<<tag() ; } // NO_COV
	void Pdate::operator>>(::string& os) const { os << "PD:"<<str(9)             ; } // NO_COV
