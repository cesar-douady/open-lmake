...
#else
	constexpr size_t hardware_destructive_interference_size = 64 ; // almost all systems have 64-bytes L1 cache lines
#endif
...
template<class V,class T> V  sum( T const& x , ::function<V(VT const&)> val , V  init={} ) { for( auto const& v : x ) init =       init+val(v)  ; return init ; }
...
template<        class T> VT sum( T const& x ,                                VT init={} ) { for( auto const& v : x ) init =       init+    v   ; return init ; }
