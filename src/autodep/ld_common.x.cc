...
#endif
{
	...
	// s_autodep_env(New) to ensure initialization
	// for HDR1_DIR, if path is empty, we may read dir provided by path.at
	#define HDR0_DIR(libcall,     args) HDR( libcall , (Record::s_autodep_env(New).readdir_ok)                                                     , args )
	#define HDR1_DIR(libcall,path,args) HDR( libcall , (Record::s_autodep_env(New).readdir_ok)&&Record::s_is_simple(path,false/*empty_is_simple*/) , args )
