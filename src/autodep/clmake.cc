...
static Ptr<> _depend( Tuple const& py_args , Dict const& py_kwds ) {
	...
	if (verbose) {
		...
		for( size_t i : iota(dep_infos.first.size()) ) {
			...
			if ( ignore_err && vi.ok==Maybe ) py_dep_info->set_item( "ok"       , Py::None                    ) ;
			if ( ignore_err && vi.ok!=Maybe ) py_dep_info->set_item( "ok"       , *Ptr<Bool>(vi.ok==Yes )     ) ;
...
template<bool Encode> static Ptr<> _codec( Tuple const& py_args , Dict const& py_kwds ) {
	...
	for( auto const& [py_key,py_val] : py_kwds ) {
		...
		switch (key[0]) {
			...
			case 't' : if (key=="table"        ) { throw_if(+tab    ,"arg ",key,MsgEnd) ; tab     =                  *py_val.str()  ; continue ; } break ;
	...
	/**/         throw_unless( +tab     , "missing arg ","table"   ) ;
	...
	try {
		::string res =
			Encode ? JobSupport::encode( ::move(*tab) , ::move(*ctx) , ::move(*cv/*val*/ ) , min_len.value_or(1) , version.value_or(0) )
			:        JobSupport::decode( ::move(*tab) , ::move(*ctx) , ::move(*cv/*code*/)                       , version.value_or(0) )
		;
...
#endif
{
	...
	,	F( "target" , _py_func<_target> ,
			...
			",\tno_warning  =False # dont warn if target is either uniquified or unlinked and generated by another rule\n"
	...
	,	F( "decode" , (_py_func<Object,_codec<false/*Encode*/>>) ,
			"decode(table,ctx,code,codec_version=0)\n"
			...
			"Codec table version is checked against codec_version (last available codec_version if 0).\n"
		)
