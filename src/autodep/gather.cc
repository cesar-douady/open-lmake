	...
	for(;;) {
		...
		for( int readdir : iota(1+ai.read_dir()) ) {
			...
			for ( size_t m : iota(star_matches.size()) ) {
				// _star_matches is sorted : Target first, then other, so for no_star accesses, listing can stop as soon as a non-Target entry is found
	...
	for( AccessEntry* access : access_seq ) {
		...
		if ( info.read_dir() && !(flags.extra_dflags[ExtraDflag::ReaddirOk]||flags.tflags[Tflag::Incremental]) ) { // if incremental, user handle previous values
			...
			if (!readdir_warned) {
				...
				res.msg << "  - avoid reading dir (including use of rm -r, grep -R, find or wildchars)\n"          ;
				res.msg << "  - if this is due to python imports, ensure "<<rule<<" derives from PyRule\n"         ;
						...
						switch (proc) {                                                                                                     // ... cannot reply on fast_report_fd
							...
							case Proc::AccessPattern :
								...
								for( auto const& [f,_] : jerr.files ) {
									...
									SWEAR( !jerr.digest.flags.tflags[Tflag::Target] , jerr ) ;                             // jerr cannot contain Target tflag and this is not supported by ...
									add_star_match( {f/*pattern*/,true/*cache*/} , ::pair(jerr.date,jerr.digest.flags) ) ; // ... add_star_match that must accumulate Target entries first
								}
							break ;
						DF}                                                                                                // NO_COV
