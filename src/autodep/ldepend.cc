...
int main( int argc , char* argv[]) {
	...
	//
	bool                                        sync      = ad.flags.dflags[Dflag::Verbose] || ad.flags.extra_dflags[ExtraDflag::Direct] ;
	...
	if (!sync) return 0 ;
	//
	if (ad.flags.dflags[Dflag::Verbose]) {
		...
		size_t   w_crc  = 3 ;
		size_t   w_rule = 0 ;
		for( DepVerboseInfo dvi : dep_infos.first ) {
			const char* ok_str ;
			switch (dvi.ok) {
		...
		Fd::Stdout.write(out) ;
		if (cmd_line.flags[Flag::IgnoreError]) return 0  ;
		else                                   return rc ;
	}
	if (ad.flags.extra_dflags[ExtraDflag::Direct]) {
		return dep_infos.second ? 0 : 1 ;
	}
	FAIL() ;
}
