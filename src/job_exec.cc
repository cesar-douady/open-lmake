...
int main( int argc , char* argv[] ) {
	...
	{	Trace trace("main",Pdate(New),::span<char*>(argv,argc)) ;
		...
		for( bool do_targets : {true,false} ) {                                                   // Target entries must be accumulated first with add_star_match
			for( auto& [dt,mf] : g_start_info.static_matches )
				if (mf.tflags[Tflag::Target]==do_targets) {
					if (mf.extra_tflags[ExtraTflag::Optional]) g_gather.add_star_match( {Re::escape(dt),false/*cache*/} , ::pair(washed,mf) )              ; // Optional are star targets with ...
					else                                       g_gather.new_access( washed , ::move(dt) , {.flags=mf} , DepInfo() , Comment::StaticMatch ) ; // ... fixed patterns
				}
			for( auto& [p,mf] : g_start_info.star_matches )
				if ( mf.tflags[Tflag::Target]==do_targets && +mf ) g_gather.add_star_match( {p,true/*cache*/} , ::pair(washed,mf) ) ;                        // fast path : dont match if no flags
		}
		//
