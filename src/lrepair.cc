...
RepairDigest repair(::string const& from_dir) {
	...
	for( ::string const& jd : walk(from_dir,from_dir) ) {
		{	JobInfo job_info { jd } ;
			...
			if (it==rule_tab.end()) { trace("no_rule",jd,job_info.start.rule_cmd_crc) ; goto NextJob ; }                // no rule
			Rule rule = it->second ;
			// find targets
...
int main( int argc , char* /*argv*/[] ) {
	...
	try { startup_s = search_root_s().startup_s ; } catch (::string const& e) { exit(Rc::Usage ,e                                                                          ) ; }
