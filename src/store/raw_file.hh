...
namespace Store {
	...
	public :
		...
		void clear(size_t sz=0) {
			...
			_dealloc() ;
			//                 vvvvvvvvvvvvvvvvvvv
			if (+_fd) _chk_rc( ::ftruncate(_fd,sz)==0 && ::lseek(_fd,0/*offset*/,SEEK_END)>=0 , "truncate" ) ; // can use ftruncate here as there is no mapping, hence no race withpage write back
			//                 ^^^^^^^^^^^^^^^^^^^
			_alloc() ;
			_map(sz) ;
		}
