	...
	void JobExec::end(JobDigest<Node>&& digest) {
		...
		if ( !lost && status>Status::Early ) {           // if early, we have not touched the targets, not even washed them, if lost, old targets are better than new ones
			...
			for( auto& [target,td] : digest.targets ) {
				...
				bool   pre_exist       = !tflags[Tflag::Incremental] && td.pre_exist ;
				//
				old_targets.erase(target) ;
				//
				if      ( pre_exist                                  )   target->polluted = Polluted::PreExist ; // pre-existed while not incremental => polluted even if generated by official job
				...
				if ( crc==Crc::None && !static_phony ) {
					...
					if ( pre_exist && !is_src ) target_reason |= {JobReasonTag::PrevTarget,+target} ;   // sources are not unlinked, hence not marked PrevTarget
		...
		if (fresh_deps) {
			...
			jd.deps.assign(deps) ;
			//^^^^^^^^^^^^^^^^^^
for( Dep const& dep : jd.deps ) *Node(dep) ;                                                              // XXX : until bug is found
		}
		//
		// wrap up
		//
