...
namespace Engine {
	...
	vmap<Node,FileAction> JobData::pre_actions( Rule::SimpleMatch const& match , bool mark_target_dirs ) const { // thread-safe
		...
		for( Target t : targets ) {
			...
			else if ( t->polluted==Yes            ) fat = FileActionTag::UnlinkPolluted ; // wash pollution
	...
	void JobExec::end( JobRpcReq&& jrr , bool sav_jrr , ::vmap_ss const& rsrcs ) {
		...
		// handle targets
		//
		for( Node t : (*this)->targets ) if (t->polluted==Maybe) t->polluted = No ; // old targets are no more busy
		//
		if ( !lost && status>Status::Early ) {                                      // if early, we have not touched the targets, not even washed them, if lost, old targets are better than new ones
			...
			for( auto const& [tn,td] : digest.targets ) {
				...
				//
				if      ( td.pre_exist             ) target->polluted = Yes   ;                   // if target pre_existed while not incremental, it is polluted even if generated by official job
				else if ( tflags[Tflag::Target]    ) target->polluted = No    ;
				else if ( +crc && crc!=target->crc ) target->polluted = Yes   ;
				if (target->polluted==Yes) trace("polluted",STR(td.pre_exist),tflags,crc,target->crc) ;
				//
				if (+crc) {
		...
		for( Req req : running_reqs_ ) {
			...
			JobReport jr = audit_end( ri , true/*with_stats*/ , pfx , job_msg , full_report&&!job_err?stderr:""s , end_none_attrs.max_stderr_len , exec_time ) ;
			if (ri.done()) {
				trace("wakeup_watchers",ri) ;
				ri.wakeup_watchers() ;
				one_done = true ;
			} else {
				req->missing_audits[*this] = { jr , msg } ;
			}
			trace("req_after",ri) ;
			req.chk_end() ;
		}
	...
	JobReport JobExec::audit_end( ReqInfo& ri , bool with_stats , ::string const& pfx , ::string const& msg , ::string const& stderr , size_t max_stderr_len , Delay exec_time ) const {
		...
		Trace trace("audit_end",color,pfx,step,*this,ri,STR(with_stats),STR(done),STR(speculate),jr,STR(+msg),STR(+stderr)) ;
	...
	Wakeup :
		if ( auto it = req->missing_audits.find(idx()) ; it!=req->missing_audits.end() && !req.zombie() ) {
			...
			JobReason r = reason() ;
			if (r.tag>=JobReasonTag::Err) audit_end( ri , true/*with_stats*/ , ja.report==JobReport::Hit?"hit_":"was_" , reason_str(r)  , stderr , max_stderr_len ) ;
			else                          audit_end( ri , true/*with_stats*/ , ja.report==JobReport::Hit?"hit_":"was_" , ja.backend_msg , stderr , max_stderr_len ) ;
	...
	bool/*maybe_new_deps*/ JobData::_submit_plain( ReqInfo& ri , JobReason reason , CoarseDelay pressure ) {
		...
		try {
			...
			//       ^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
			for( Node t : targets ) if (t->polluted==No) t->polluted = Maybe ;                                // make targets busy once we are sure job is submitted
