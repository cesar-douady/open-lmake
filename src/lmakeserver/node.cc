...
namespace Engine {
	...
	bool/*modified*/ NodeData::refresh_src_anti( bool report_no_file , Accesses a , ::vector<Req> const& reqs_ , ::string const& name_ ) { // reqs_ are for reporting only
	...
	Codec :
		...
		FAIL() ;                                                                                            // NO_COV
	Src :
		{	bool modified = refresh_src_anti( status()!=NodeStatus::None , ~Accesses() , {req} , lazy_name() ) ;
	...
	Manual NodeData::manual_refresh( Accesses a , Req req , FileSig const& sig ) {
		Manual m = manual(a,sig) ;
		...
		if (  crc_!=     crc            ) { crc = {} ; date() = sig_ ; crc = crc_ ; }            // defensive programming : ensure crc and date are never incoherent
		else                                           date() = sig_ ;
