...
namespace Engine {
	...
	NoSrc :
		{	if (ri.goal>=NodeGoal::Dsk) {
				...
				if (crc!=Crc::None) {
					refresh(Crc::None) ;                                                                // if not physically unlinked, node will be manual
					polluted     |= +actual_job() ;                                                     // disk has been modified, actual_job cannot be the official job
					actual_job()  = {}            ;
				}
				goto Done ;
			}
	...
	void NodeData::mk_old() {
		...
		if ( +actual_job() && actual_job()->rule.old() ) {
			...
			polluted     = true ;                          // although actual_job does not reflect it, job has been generated by a job which cannot be official any more
		}
		_set_match_gen(false/*ok*/) ;
	}
