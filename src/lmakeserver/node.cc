...
namespace Engine {
	...
	Manual NodeData::manual_wash( ReqInfo& ri , bool query , bool dangling ) {
		...
		bool   stamp    = true                ;
		dangling &= !( polluted!=No || +actual_job() ) ; // if polluted, we have been generated by a job
		switch (res) {
			case Manual::Ok      :
	...
	NoSrc :
		{	if (ri.goal>=NodeGoal::Dsk) {
				...
				if (crc!=Crc::None) {
					refresh(Crc::None) ;                                                                // if not physically unlinked, node will be manual
					polluted     = polluted|+actual_job() ;                                             // disk has been modified, actual_job cannot be the official job
					actual_job() = {}                     ;
				}
				goto Done ;
			}
	...
	static bool _may_need_regenerate( NodeData const& nd , NodeReqInfo& ri , NodeMakeAction make_action ) {
		...
		/**/                                if (nd.polluted!=No                       ) ri.done_ &= NodeGoal::Status ; // disk cannot be ok if node was polluted, does not change conform_job_tgt()
			...
			{	ReqInfo::WaitInc sav_n_wait{ri} ;                                                        // ensure we appear waiting while making jobs to block loops (caught in Req::chk_end)
				for(; it ; it++ ) {
					...
					if (query)
						...
						case NodeGoal::Dsk     :
							if      (polluted!=No && crc==Crc::None   ) reason = {JobReasonTag::NoTarget      ,+idx()} ;
							else if (polluted==Maybe                  ) reason = {JobReasonTag::BusyTarget    ,+idx()} ;
							else if (polluted==Yes                    ) reason = {JobReasonTag::PollutedTarget,+idx()} ;
								...
								switch (manual_wash(ri,false/*query*/,false/*dangling*/)) {
	...
	void NodeData::mk_old() {
		...
		if ( +actual_job() && actual_job()->rule.old() ) {
			...
			polluted     = Yes  ;                          // although actual_job does not reflect it, job has been generated by a job which cannot be official any more
		}
		_set_match_gen(false/*ok*/) ;
	}
