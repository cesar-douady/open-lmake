...
namespace Engine {
	...
	Manual NodeData::manual_wash( ReqInfo& ri , bool query , bool dangling ) {
		...
		Req    req   = ri.req                           ;
		Manual res   = manual_refresh(FullAccesses,req) ;
		bool   stamp = true                             ;
	...
	bool/*modified*/ NodeData::refresh_src_anti( ::string const& name_ , Accesses a , ::vector<Req> const& reqs_ , bool report_no_file ) { // reqs_ are for reporting only
		...
		Trace trace("refresh_src_anti",idx(),STR(report_no_file),a,reqs_,sig) ;
		...
		if (!fi.exists()) {
			...
			if (crc.valid()) { //!                                       updated
				if (     sig       ==         date.sig           ) return false ;                                                          // sig's match
				if ( Crc(sig.tag()).match(Crc(date.sig.tag()),a) ) return false ;                                                          // sig's are enough to match content
			}
			Crc      crc_     { name_ , /*out*/sig }    ;
			Accesses mismatch = crc.diff_accesses(crc_) ;
			//vvvvvvvvvvvvvvvvvvvvvvvv
			set_crc_date( crc_ , sig ) ;
			//^^^^^^^^^^^^^^^^^^^^^^^^
	...
	bool/*solved*/ NodeData::_make_pre( ReqInfo& ri , bool query ) {
		...
		FAIL() ;                                                                                          // NO_COV
	Src :
		{	bool modified = refresh_src_anti( lazy_name() , FullAccesses , {req} , status()!=NodeStatus::None ) ;
	...
	bool/*modified*/ NodeData::set_crc_date( Crc crc_ , SigDate const& sd ) {
		...
		//
		Trace trace( "set_crc_date" , STR(modified) , idx() , crc ,"->", crc_ , date ,"->", sd ) ;
		//
		if (crc_!=crc) { crc = {} ; date = sd ; crc = crc_ ; }               // defensive programming : ensure crc and date are never incoherent
		else                        date = sd ;
		for( Req r : reqs() ) {
	...
	Manual NodeData::manual_refresh( Accesses a , Req req ) {
		if (buildable==Buildable::Codec) {
			if (crc==Crc::None) {                                                           // may be created up-to-date remotely as codec table file is (logically) updated simultaneously
				::string n   = name() ;
				FileSig  sig { n }    ;
				if (sig.exists()) set_crc_date( Crc(n) , sig ) ;
			}
			return Manual::Ok ;
		}
