...
try    : has_slurm = sp.check_output(('scontrol','ping'),universal_newlines=True).endswith('UP')
except : has_slurm = False
...
class GenCacheLight(BaseRule,PyRule) :
	target = '{File}-light.cc'
	def cmd() :
		sys.stdout.write(textwrap.dedent(f'''
			#define CACHE_LIGHT 1
			#include "{File}.cc"
		''')[1:])
