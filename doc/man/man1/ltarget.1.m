...
.SH DESCRIPTION
.LP
B_(ltarget) may be used to pass flags to OpenLmake.
...
Bullet
	Targets are reported even if the autodep method (the I_(autodep) rule attribute) is I_(none).
	This is B_(the) way (or a call to B_(lmake.target)) of reporting targets in such a case (usually with I_(--write)).
.LP
...
Item(B_(-W),B_(--write))           Report an actual write, not only target flags. Default is to only alter flags.
Item(B_(-z),B_(--dir))             Specify a dir for use with I_(--list).
Item(B_(-l),B_(--list))            Print list of currently generated targets to stdout.
Only targets lying in the dir mentioned with I_(--dir) are listed (if this option is used), and only if they match the regexpr argument (if I_(--regexpr)) as reported.
...
Cf note (3).
Item(B_(-X),B_(--regexpr))         Pass flags to all targets matching regexprs passed as argument. The B_(ignore) flag only applies to targets following this command.
Item(B_(-x),B_(--no-exclude-star)) Accept that flags are further processed according to regexpr-based requests, e.g. B_(ltarget --regexpr), default is to exclude such processing.
Item(B_(-E),B_(--essential))       Show when generating user oriented graphs.
Item(B_(-i),B_(--incremental))     Targets are not unlinked before job execution and read accesses to them are ignored (including C_(readdir,3)).
Item(B_(-w),B_(--no-warning))      No warning is emitted if targets are either uniquified or unlinked while generated by another job.
Item(B_(-`I'),B_(--ignore))        From now on, ignore all reads and writes to targets (including C_(readdir,3)).
Item(B_(-a),B_(--no-allow))        Unless this option is passed, B_(ltarget) makes its arguments valid targets.
Item(B_(-s),B_(--source-ok))       Unless this option is passed, writing to a source is an error. In that case, being simultaneously a dep and a target is ok.
.LP
In case mentioned targets turn out to be deps, the dep flags are also available:
Item(B_(-c),B_(--critical))     Create critical deps (cf. note (4)).
...
.SH "EXIT STATUS"
.LP
B_(ltarget) exits with a status of zero if target flags were correctly set.
...
.SH EXAMPLES
.LP
V_(ltarget --ignore a_file) # reading/writing to a_file will be ignored
.LP
V_(echo >a_file)
.LP
V_(ltarget --readdir_ok a_dir)
.LP
V_(rm -rf a_dir)

.SH NOTES
Item((1))
	The same functionality is provided with the B_(lmake.target) python function.
Item((2))
	Flags can be associated to targets on a regexpr (matching on target name) basis by using the B_(side_targets) rule attribute.
Item((3))
