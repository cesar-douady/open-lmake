...
.SH DESCRIPTION
.LP
B_(lmake_repair) is meant to repair internal OpenLmake book-keeping in case of catastrophic crash, either because of system failure or a bug in OpenLmake.
...
In such a case, OpenLmake will suggest the use of B_(lmake_repair).
.LP
In case of incoherent behavior, the user may decide to run B_(lmake_repair).
.LP
...
.SH "EXIT STATUS"
.LP
B_(lmake_repair) exits with a status of zero if the repo could be successfully repaired.
