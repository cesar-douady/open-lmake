...
.IP
If I_(--porcelain), the output is generated as an easy to parse B_(dict).
...
.IP
If I_(--porcelain), the output is generated as a B_(str) (not indented).
...
.IP
If I_(--porcelain) and args are files, the output is, for each file, a B_(dict) indexed by jobs and whose values are as follows.
...
.IP
If I_(--porcelain), the output is a B_(set) of jobs, each job being represented as a B_(tuple) composed of the rule name and the job name.

Item(B_(-E),B_(--env))
Show the environment used to run the script
.IP
If I_(--porcelain), the output is generated as as B_(dict), much like B_(os.environ).
...
Show various info about a job, as it last ran (unless stated otherwise):
	...
	Bullet B_(external_read_ok)   : true if reading outside repo and source dirs is allowed.
	Bullet B_(external_write_ok)  : true if writing outside repo is allowed.
	...
	.IP
	If I_(--porcelain), the output contains the same information generated as a B_(dict).
...
.IP
If I_(--porcelain), whether args are a job or files, the output is an easy to parse B_(dict).
...
Output is always generated with a single tab prefix for each line, so as to maintain internal alignment.
.IP
If I_(--porcelain), the output is generated as a B_(tuple) composed of a message generated by lmake at start time, a message generated by lmake at en time and the stderr of the job.
...
Unless I_(--quiet), output is preceded by a description of the job which it relates to.
.IP
If I_(--porcelain), the output is generated as a B_(str).
...
.IP
If I_(--porcelain), the output is generated as a B_(set) of targets,
...
.IP
If I_(--porcelain), the output is generated as a B_(tuple) of entries, each entry being represented as a tuple composed of a date, a tag and a file (as described above), each being a B_(str).

SpecificOptions
Item(B_(-p),B_(--porcelain))
In porcelain mode, information is provided as an easy-to-parse python object.
Also,reported files are relative to the root of the repo, not the current working dir.
.IP
