...
Title(autodep,mimic OpenLmake job prelude)

.SH SYNOPSYS
B(autodep) [I(OPTION)]... I(cmd) I(args)...

.SH DESCRIPTION
.LP
B(autodep) is used by B(ldebug) to launch jobs in an adequate environment.
...
.LP
The trace generated by B(autodep) is halfly digested in the sens that :
Bullet
...
Item(B(-r) I(abs_dir),B(--root-view)=I(abs_dir))
Mimic setting the rule attribute B(root_view)=I(abs_dir).
