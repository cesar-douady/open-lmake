Comment( This file is part of the open-lmake distribution (git@github.com:cesar-douady/open-lmake.git)                                               )
Comment( Copyright (c) 2023 Doliam                                                                                                                   )
Comment( This program is free software: you can redistribute/modify under the terms of the GPL-v3 (https://www.gnu.org/licenses/gpl-3.0.html).       )
Comment( This program is distributed WITHOUT ANY WARRANTY, without even the implied warranty of MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE. )
.TH LMAKE 1 "System(`date "+%d %B %Y"')" "Doliam" "User Commands"
.SH NAME
lmake \- a reliable & scalable tool to execute a full flow

.SH SYNOPSIS
B(lmake) [I(OPTION)]... [I(TARGET)]...
...
.IP Bullet
Recipies can be written in B(bash) (1) or B(python) (1).
.IP Bullet
...
.LP
B(lmake) manages a fully coherent directory called a repository.
...
.TP
B(-r) I(count), B(--retry-on-error)=I(count)
...
It is not colored.
.IP Bullet
Reported files are relative to the root of the repository, not to the current working directory where the B(lmake) command has been launched.
.LP
The log contains a line, possibly followed by attached information when the following events occur :
.IP Bullet
A job is started, if the job duration is longer than the I(start_delay) attribute of the rule.
.IP Bullet
A job is completed.
.IP Bullet
...
.IP Bullet
B($PATH) is reset to the default path for the system, plus the I(open-lmake) bin directory.
.IP Bullet
B($PYTHONPATH) is set to the I(open-lmake) lib directory.
.IP Bullet
...
If a single one is found, this determines the root of the repository.
In other cases, B(lmake) will not start.

.SH "SEE ALSO"
.LP
B(autodep)(1),
B(find_cc_ld_library_path)(1),
B(lcheck_deps)(1),
B(ldebug)(1),
B(ldecode)(1),
B(lencode)(1),
...
.IR "open-lmake" .
.LP
B(open-lmake) is free software; you can redistribute it and/or modify it under the
terms of the GNU General Public License as published by the Free Software
Foundation, version 3 of the License.
.LP
B(open-lmake) is distributed in the hope that it will be useful, but WITHOUT ANY
